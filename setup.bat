...
if exist "%INSTALL_DIR%main.py" (
    echo [INFO] Detectado repo existente en: %INSTALL_DIR%
    goto :run_setup_ps1
)
...
if errorlevel 1 (
    ...
    echo  Instala "Instalador de aplicacion" desde Microsoft Store y volvé a correr este script.
    echo.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    ...
    git --version >nul 2>&1
    if errorlevel 1 (
        echo   FAIL: Git instalado pero no disponible aun. Cerra y volvé a abrir la terminal.
        pause
        exit /b 1
    )
)
...
if "!PY_OK!"=="0" (
    ...
    if errorlevel 1 (
        echo   FAIL: Python instalado pero no disponible aun. Cerra y volvé a abrir la terminal.
        pause
        exit /b 1
    )
)
...
if exist "%INSTALL_DIR%.git" (
    ...
    if exist "%INSTALL_DIR%" (
        echo   WARN: %INSTALL_DIR% existe pero no es un repo git. Clonando de todas formas...
    )
    git clone https://github.com/pabloeckert/MejoraRedmi14c.git "%INSTALL_DIR:~0,-1%"
    if errorlevel 1 (
        echo   FAIL: No se pudo clonar el repositorio. Verificá tu conexion a internet.
        pause
        exit /b 1
    )
...
echo.
echo ===================================================
echo  Setup completo. El OTA watcher ya esta corriendo.
